...
int temp_tempo[] = {                                               //Define the note durations, 1 to 1 with melody    1 = 8 beats 
...
void sing(int s) {
  ...
  if (song == 72) {
    ...
    int size = sizeof(temp_melody) / sizeof(int);
    for (int thisNote = 0; thisNote < size; thisNote++) {

      // to calculate the note duration, take one second
      // divided by the note type.
      //e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
      int noteDuration = 1000 / temp_tempo[thisNote];

      buzz(melodyPin, temp_melody[thisNote], noteDuration);
